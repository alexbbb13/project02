...
const query = 'select filename from photo_users left join photos on photos.id=photo_users.photo_id where photo_users.user_id=$1::int';
const params = [id];
pool.query(query,params, function(err, result) {
    ...
    if(record.length > 0) {
            const recordMapped = record.map(row => { 
                ...
                return rObj;});  
            res.render('photos', {photos: recordMapped});
        } else {
